...
struct WADInputStreamGIO : WAD::WADInputStream {
  ...
  virtual uint32_t read_uint32() {
    uint8_t raw[4];
    read_bytes(raw, 4);
    uint32_t num = (raw[0] | (raw[1] << 8) | (raw[2] << 16) | (raw[3] << 24));
    return num;
  }
 ...
 */
static std::string
generate_unique_name(std::filesystem::path const &wad_path,
...
TextureManager::get_wad_paths() const {
...
TextureManager::get_texture(std::string const &name) const {
