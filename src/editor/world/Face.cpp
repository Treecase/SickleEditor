...
public:
    ...
    bool operator()(MAP::Vertex const &a, MAP::Vertex const &b) const
    {
        ...
        MAP::Vector2 const a_proj{
            glm::dot(a_local, u_axisN),
            glm::dot(a_local, v_axisN)};
        MAP::Vector2 const b_proj{
            glm::dot(b_local, u_axisN),
            glm::dot(b_local, v_axisN)};
        ...
        if (a_theta < 0.0f)
        {
            a_theta += glm::radians(360.0);
        }
        if (b_theta < 0.0f)
        {
            b_theta += glm::radians(360.0);
        }
